...
#         freeze(self.mlps)
#         freeze(self.layers)
#         freeze(self.project_in)
#         freeze(self.project_out)
#
...
class ResidualVQ(Module):
    ...
    def __init__(
        ...
        self.layers = ModuleList([VectorQuantize(dim=codebook_dim, codebook_dim=codebook_dim,
                                                 accept_image_fmap=accept_image_fmap, **vq_kwargs) for _ in
...
class GroupedResidualVQ(Module):
    ...
    def forward(
        ...
        out = tuple(rvq(chunk, indices=chunk_indices, **forward_kwargs) for rvq, chunk, chunk_indices in
                    zip_longest(self.rvqs, x, indices))
        out = tuple(zip(*out))
