...
def create_mask(a_len, b_len, device, step, rst=0):
    st = np.random.randint(rst, 11) * step
    mask_seq = torch.rand(a_len) * (b_len - st) + st
    ...
    mask_a[mask < .5] = float(-torch.inf)
    mask = torch.rand_like(mask_b)
    mask_b[mask < .5] = float(-torch.inf)

    mask_a = F.pad(mask_a, (0, 1), "constant", 0)
    mask_b = F.pad(mask_b, (0, 1), "constant", 0)
...
class Sholace(nn.Module):
    ...
    def forward(self, audio_seq, midi_seq, melody_seq, desc):
        ...
        if np.random.rand() > .5:
            audio_attn_mask, midi_attn_mask = create_mask(a_len, b_len, audio_seq.device, b_len // 10, rst=9)
        else:
            midi_attn_mask, audio_attn_mask = create_mask(b_len, a_len, audio_seq.device, a_len // 10)
    ...
    @torch.no_grad
    def inference(self, midi_seq, audio_seq, desc,
                  midi_top_k, audio_top_k, prompt_len=4 * 50, mode="a2b"):
        ...
        if mode == "a2b":
            ...
            if midi_seq.shape[1] > 49:
                midi_seq = midi_seq[:, :49]
            ...
            for offset in tqdm(range(start_offset_sequence, gen_sequence_len),
                ...
                gen_sequence = self.audio_lm.sample_next_tokens(logits, mask, offset,
                                                                gen_sequence, top_k=audio_top_k)
            audio_pred = self.audio_lm.decode(gen_sequence, max_gen_len)
            return audio_pred
            ...
            condition_tensors, attributes, gen_sequence, mask, start_offset_sequence \
                = self.audio_lm.prepare_for_infer(desc=desc,
                                                  prompt=audio_seq[:, :prompt_len],
                                                  max_gen_len=max_gen_len,
                                                  num_samples=len(audio_seq),
                                                  device=audio_seq.device)
            gen_sequence_len = gen_sequence.shape[-1] // 16
            gen_sequence = gen_sequence[..., :max_gen_len]
            midi_pred = midi_seq[:, :prompt_len // 16]

            for offset in tqdm(range(start_offset_sequence, gen_sequence_len),
                               total=gen_sequence_len,
                               desc=f"inference"):

                for midi_onset in range(1):
                    self.cache = self.midi_lm.encode2roll(midi_pred, n=len(audio_seq))
                    self.cache["audio_attn_mask"] = None
                    self.cache["midi_attn_mask"] = None
                    self.hidden_states = {}
...
class Yingyang(nn.Module):
    ...
    def inference(self, midi_seq, audio_seq, desc, mode, midi_top_k, audio_top_k):
        with torch.no_grad():
