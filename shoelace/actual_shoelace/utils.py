...
def crop_midi(seq, chunk_len, hop_len, align_tail=False):
    ...
    gen_seq = []
    prompts = []
    min_len = max([suffix_idx[i] - prefix_idx[i] for i in range(len(suffix_idx))])
    # + len(prefix[i])
    for i in range(len(seq)):
...
def crop_midi_and_align(midi_seq, start_idx, min_chunk_len):
    _, prompts = crop_midi(midi_seq, chunk_len=start_idx + min_chunk_len, hop_len=start_idx, align_tail=True)
    prompts = torch.stack(prompts, 0)
    index, _ = make_index(midi_seq, chunk_len=0)
