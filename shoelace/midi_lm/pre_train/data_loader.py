...
class MIDIDataset(BaseDataset):
    def __init__(self, path_folder: str, rid: int, num_workers: int = 1, validation: bool = False):
        super().__init__()

        self.rid = rid
        self.use_loader = num_workers > 1
        self.files, self.feature_paths = load_data_lst(path_folder, validation=validation)
