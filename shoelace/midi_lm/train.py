...
from shoelace.midi_lm.data_loader import MIDIDataset as Dataset
from shoelace.midi_lm.data_loader import collate_fn, worker_init_fn
from shoelace.midi_lm.models import MIDILM as Model
...
def train_dist(replica_id, replica_count, port, model_dir, args):
    ...
    from shoelace.midi_lm.config import midi_lm_param, baby_param
    model = Model(param=midi_lm_param,
                  baby_param=baby_param)
    model.load_state_dict(torch.load("exp/midi_lm/latest_1_9000.pth.new_version.new_version", map_location="cpu"),
                          strict=False)
