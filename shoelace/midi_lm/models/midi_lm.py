...
class MIDILM(nn.Module):
    ...
    def inference(self, x, max_len=512, top_k=32, temperature=1.0):
        ...
        for _ in tqdm(range(max_len - prompt_len), desc="Inference", total=max_len - prompt_len):
            ...
            attn_mask = generate_attention_mask(seq_len=embedding.shape[1],
                                                attn_window=512,
                                                mask_prob=0.7, device=embedding.device)

            decoder_output = self.transformer_decoder(embedding, is_causal=False, mask=attn_mask)
