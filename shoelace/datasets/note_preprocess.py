import os
from multiprocessing import Process
import pretty_midi
import numpy as np
import h5py


def midi_to_matrix(path, res=50):
    try:
        midi_data = pretty_midi.PrettyMIDI(path)
    except:
        return None
    max_time = -1
    outs = []

    for instrument in midi_data.instruments:
        assert not instrument.is_drum
        for note in instrument.notes:
            outs.append([note.pitch, note.start, note.end, note.velocity])
            if max_time < note.end:
                max_time = note.end

    if max_time < 30:
        return None

    max_frame = round(max_time * res) + 2
    onsets = np.zeros([max_frame, 128])
    activations = np.zeros([max_frame, 128])

    for line in outs:
        pitch, start, end, velocity = line
        start = int(start * res)
        end = int(end * res)
        if start >= end:
            continue
        activations[start:end, int(pitch)] = velocity
        onsets[start: start + 1, int(pitch)] = 1000
    data = activations + onsets
    data = np.unique(data, axis=0)
    index = np.arange(len(data))
    np.random.shuffle(index)
    data = data[index]
    return data


def extract_feature_unit(output_path, lines):
    with h5py.File(output_path, "w") as hf:
        for line in lines:
            data = midi_to_matrix(line, res=50)
            if data is None:
                print(line)
                continue
            hf.create_dataset(line, data=data.astype(np.int16))


def extract_feature(file_list_folder, output_feature_folder):
    ps = []
    for fname in os.listdir(file_list_folder):
        file_list_path = os.path.join(file_list_folder, fname)
        with open(file_list_path, "r") as f:
            lines = f.readlines()
        lines = [line.rstrip() for line in lines]
        output_path = os.path.join(output_feature_folder, str.replace(fname, ".lst", ".h5"))
        p = Process(target=extract_feature_unit, args=(output_path, lines))
        p.start()
        ps.append(p)

    for p in ps:
        p.join()


if __name__ == "__main__":
    file_list_folder = "data/formatted/groups/text"
    output_feature_folder = "data/formatted/groups/note_feature_full"
    os.makedirs(file_list_folder, exist_ok=True)
    os.makedirs(output_feature_folder, exist_ok=True)
    extract_feature(file_list_folder, output_feature_folder)
