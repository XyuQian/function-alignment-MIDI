            ...
            with h5py.File(data_path, "r") as hf:
                ...
                for j, f in tqdm(enumerate(files[i]), total=len(files),
                    ...
                    if total_len > seg_len:
                        for k in range(0,  total_len - seg_len, 1):
...
def collate_fn(batch):
    ...
    for b in batch:
        ...
        chords = np.pad(b[2], (0, max_t - b[2].shape[0]), 'constant', constant_values=(320, 320))
        outs.append([piano, melody, chords])
    x = torch.from_numpy(np.stack([out[0] for out in outs], 0)).long()
    melody = torch.from_numpy(np.stack([out[1] for out in outs], 0)).long()
    chords = torch.from_numpy(np.stack([out[2] for out in outs], 0)).long()
    return {
        "x": x,
        "melody": melody,
        "chords": chords
