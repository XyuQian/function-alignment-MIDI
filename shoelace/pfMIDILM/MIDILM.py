...
class MIDILM(nn.Module):
    ...
    def yield_forward(self, midi_seq, cut_seq=True, return_memory=False):
        x = midi_seq
        if cut_seq:
            input_x = F.pad(x[:, :-1], (0, 0, 1, 0), "constant", SOS)
            baby_input_x = F.pad(x[:, :, :-1], (1, 0), "constant", SOS)
        else:
            input_x = F.pad(x, (0, 0, 1, 0), "constant", SOS)
            baby_input_x = F.pad(x, (1, 0), "constant", SOS)
        # print(input_x.shape, baby_input_x.shape)
        embed_x = self.input_embedding(input_x)
        embed_x_with_pos = self.pos_encoding(embed_x)
        attn_mask = nn.Transformer.generate_square_subsequent_mask(input_x.shape[1]).to(x.device)
        src_padding_mask = torch.where(input_x[:, :, 0] == PAD, float('-inf'), 0) if x is not None else None
